...
def calculate_pnl(player: Player, fv: int):
    if player.stock > 0:
...
if __name__ == "__main__":
    ...
    #     market_maker.stock += market_maker.stock
    #     market_maker.pnl += market_maker.cash
    calculate_pnl(market_maker, fv)
    sleep(0.5)
    print(f"Total pnl: >>>||  {market_maker.pnl}  ||<<<")
    for opp in opponents:
        print(f"Calculating Player {opp.identifier} pnl:")
        calculate_pnl(opp, fv)
        print(f"Player {opp.identifier} with pnl: >>>|| {opp.pnl} ||<<<")
        sleep(0.5)

    rank_pnl = [(opp.pnl, opp.identifier) for opp in opponents]
    rank_pnl.append((market_maker.pnl, market_maker.identifier))
    rank_pnl = sorted(rank_pnl)
    print(f"The pnl ranking is as follow")
    for idx, rankpnl in enumerate(rank_pnl):
        pnl, player = rankpnl
        print(f"{idx}th place is: -- Player {player} with pnl : {pnl}")
    print("+" * 60)
